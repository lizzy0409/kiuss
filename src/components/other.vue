...
<script>
 ...
 module.exports = {
    data: function() {
      return {
        otherJSON: 'src/assets/jsons/other.json', //Адрес к массиву json-данных постов
        otherArray: [], //Сюда записывается ответ (response) от get-запроса
      }
    },
    ...
    methods: {  
      ...
      getAllPosts: function() {//Метод получения ВСЕХ данных из json-файла
        ...
        this.$http.get(this.otherJSON).then(function(response) {
          this.otherArray = response.data}, function(error){})
      },
