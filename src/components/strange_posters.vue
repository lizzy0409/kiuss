...
<pre class="annotation_en" style="text-align: left;">A series of street posters (about 60 pieces of A0 format), which is, in fact, a brief manifesto of the project of <a href="http://stranno.su/#/absurd_inscriptions">absurd inscriptions</a>.
      ...
      <div class="strange_posters" v-for="(strange_posters, index) in strange_postersArray" v-bind:index="index" v-bind:key="strange_posters.index"> <!-- При использовании v-for на компонентах обязательно указывать key и явно использовать входные параметры -->
        <h3>{{strange_posters.title}}</h3> <!-- Заголовок -->
        <pre class="ru none" style="text-align: left; opacity:0;" v-html="strange_posters.body"></pre> <!-- Текст -->
        <pre class="en" style="text-align: left;" v-html="strange_posters.ebody"></pre>
        <div v-lazy-container="{ selector: 'img' }">
          <img v-bind:data-src="strange_posters.img"> <!-- Картинка -->
        </div>
      </div>
    </div> 
  </div>
 ...
 module.exports = {
    ...
    methods: {  
      ...
      getAllPosts: function() {//Метод получения ВСЕХ данных из json-файла
        ...
        this.$http.get(this.strange_postersJSON).then(function(response) {
          this.strange_postersArray = response.data}, function(error){})
      },
